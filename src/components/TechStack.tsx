...
const TechStack: React.FC<TechStackProps> = ({ data }) => {
  ...
  const getProgressGradient = (level: number) => {
    if (level >= 90) {
      return 'from-emerald-500 to-green-500'; // Excellent (90-100%)
    } else if (level >= 80) {
      return 'from-green-500 to-lime-500'; // Very Good (80-89%)
    } else if (level >= 70) {
      return 'from-lime-500 to-yellow-500'; // Good (70-79%)
    } else if (level >= 60) {
      return 'from-yellow-500 to-orange-500'; // Fair (60-69%)
    } else if (level >= 50) {
      return 'from-orange-500 to-red-500'; // Basic (50-59%)
    } else {
      return 'from-red-500 to-red-600'; // Learning (0-49%)
    }
  };
    ...
    <section className="py-12 sm:py-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-6 gradient-text">Tech Stack</h2>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <div className="glass-effect rounded-modern-lg p-2 flex flex-wrap gap-2">
        ...
        >
          {categories[activeCategory as keyof typeof categories].skills.map((skill: any, index: number) => (
            <motion.div
              key={skill.name}
              className="glass-effect rounded-modern-xl p-4 sm:p-6 card-hover group"
            ...
            >
              ...
              <div className="relative">
                {/* Background Track */}
                <div className="w-full bg-primary-elevated rounded-full h-3 sm:h-4 overflow-hidden">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <div className="glass-effect rounded-modern-xl p-6 sm:p-8">
            <h3 className="text-lg sm:text-xl font-bold mb-6 text-center gradient-text">
              Proficiency Scale
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
              {[
                { range: '90-100%', label: 'Expert', color: 'from-emerald-500 to-green-500' },
                { range: '80-89%', label: 'Advanced', color: 'from-green-500 to-lime-500' },
                { range: '70-79%', label: 'Proficient', color: 'from-lime-500 to-yellow-500' },
                { range: '60-69%', label: 'Intermediate', color: 'from-yellow-500 to-orange-500' },
                { range: '50-59%', label: 'Basic', color: 'from-orange-500 to-red-500' },
                { range: '0-49%', label: 'Learning', color: 'from-red-500 to-red-600' },
              ].map((item, index) => (
                <motion.div
                  key={index}
        ...
        >
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {['Learning: Rust', 'Exploring: Web3', 'Interested in: AI/ML'].map((item, index) => (
              <motion.span
                key={index}
                className="bg-gradient-to-r from-blue-500/20 to-purple-500/20 border border-blue-500/30 px-3 sm:px-4 py-2 rounded-full text-xs sm:text-sm text-blue-400 micro-interaction"
